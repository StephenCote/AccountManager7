(function(){
    ...
    function newTreeComponent(){
        ...
        async function inlineAdd(data){
            ...
            //let newGrp = am7model.newPrimitive(mtx.type.toLowerCase());
            //let newGrp = am7model.newPrimitive(mtx.type.toLowerCase());
            //console.log(data);
        ...
        function addNew(){
            //let path = "/new/group/" + selectedNode;
        ...
        function getTreeView(){
            ...
            if(origin){
                ...
                if(olioMode){
                    if(!olio && !checkOlio){
                        checkOlio = true;
                        page.findObject("auth.group", "DATA", "/Olio/Universes").then( (g)=> {11
                            if(!g){
                                return;
                            }
        ...
        function getMatrix(node){
            ...
            if(!treeMatrix[node.objectId]){
                ...
                if(bGroup){
                    //ico = "folder_off";
                    //let path = node.path.replace(/^\/Home\/[A-Za-z0-9\.\s@]+\//,"").replace(/^~\//, "");
                    //console.log(node);
        ...
        function getTreeViewNode(node){
            ...
            let bExpand = false;
            //if(node.objectId == origin.objectId && !treeMatrix[node.objectId]) bExpand = true;
            ...
            let itemProps = {
                ...
                /*
                ondragover: function(e){ doDragOver(e, node);},
                ondragend: function(e){ doDragEnd(e, node);},
                ondragstart: function(e){ doDragStart(e, node);},
                ondrop: function(e){ doDrop(e, node);}
                */
            };
            //if(dnd && dnd.doDragDecorate){
        ...
        function resetNode(objectId, skipList){
            if(treeMatrix[objectId]){
                ...
                */
                //let gid = mtx.groupId || mtx.parentId || 0;
