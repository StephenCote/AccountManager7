  ...
  function newListControl(){
    ...
    async function openFavorites(){
      ...
      if(embeddedMode || pickerMode){
        ...
        navContainerId = fav.objectId;
        if(byParent) navigateByParent = true;
        m.redraw();
      }
      else{
        m.route.set("/list/" + (containerMode ? baseListType : ltype) + "/" + fav.objectId, {key: Date.now()});
      }
    }
      ...
      function displayList(){
        ...
        if(rset) results = rset.map((p)=>{
          ...
          if(p.profile && p.profile.portrait && p.profile.portrait.contentType){
            ...
            icon = m("span", {class: "material-symbols-outlined material-icons-48" + (icoCls ? " " + icoCls : "")},ico);
          }
          let cls = "result-item";
          let icoTxt = "check";
          let dcls;
