/**
 * formDef.js — Form definitions for all model types (ESM)
 * Port of Ux7 client/formDef.js
 * Registers am7model.forms.* with ~100+ named form schemas
 *
 * Late-bound globals (wired in main.js):
 *   am7model._client  — replaces global am7client
 *   am7model._page    — replaces global page
 *   am7model._view    — replaces global am7view
 *
 * External dependencies (not yet ported):
 *   am7model._olio    — am7olio.dressUp / am7olio.dressDown in apparel form
 *   am7model._sd      — am7sd.fetchModels for SD model list refresh
 *   am7model._appPath — g_application_path for voice test endpoint
 */
...
import { am7model } from './model.js';
    ...
    */
    ...
    async function cleanupTestVoice(data){
        am7model._page.toast("info", "Cleaning up test voice data for " + data.objectId);
        await am7model._page.deleteObject(data[am7model.jsonModelKey], data.objectId);
    }

    async function testVoice(object, inst, name) {
        let refId = "Test Voice - " + am7model._page.uid();
        ...
        if(inst.api.engine() == "xtts" && inst.api.voiceSample()){
            ...
            let q = am7model._client.newQuery("data.data");
            q.entity.request = am7model._view.viewFields(sinst);
            q.field("organizationId", am7model._page.user.organizationId);
            q.field("objectId", inst.api.voiceSample().objectId);
            let sdr = await am7model._page.search(q);
            let sd;
            ...
            if(!sd || !sd.dataBytesStore || sd.dataBytesStore.length == 0){
                am7model._page.toast("error", "No voice sample data found for " + inst.api.voiceSample().objectId);
                return;
            }
            vprops.voice_sample = sd.dataBytesStore;
        }

        // NOTE: am7model._appPath replaces g_application_path
        let d = await m.request({method: 'POST', url: am7model._appPath + "/rest/voice/" + refId, withCredentials: true, body: vprops});
        inst.api.sampleAudio(d);
        if(d){
            am7model._page.toast("success", "Voice synthesis test successful");
            /// If there is already an audio component, then update it:
            let aud = document.querySelector("#sampleAudio");
            if(aud){
                let apath = am7model._appPath + "/media/" + am7model._client.dotPath(am7model._client.currentOrganization) + "/data.data" + d.groupPath + "/" + d.name;
            ...
            am7model._page.toast("error", "Voice synthesis test failed");
        }
        m.redraw();
    }
    ...
    async function narrate(object, inst) {
        if (!inst || inst.model.name != "olio.charPerson") {
            am7model._page.toast("warn", "Invalid object instance");
            return;
        }
        ...
        if (x && x != null) {
            ...
            am7model._page.toast("error", "Failed to update narrative");
        }
    }
    ...
    async function createCharacter(obj, profUrl){
        ...
        let cdir = await am7model._page.makePath("auth.group", "data", "~/Characters");
        let char = await am7model._page.searchFirst("olio.charPerson", cdir.id, obj.name);
        ...
        if(char == null){
            ...
            char = await am7model._page.createObject(charN);

            if(char != null){
                char = await am7model._page.searchFirst("olio.charPerson", cdir.id, obj.name);
                if(char == null){
        ...
        if(!char.profile.portrait){
            ...
            let dir = await am7model._page.makePath("auth.group", "data", "~/Gallery");
            if(!profUrl){
            ...
            let img;
            try{
                img = await am7model._page.openObjectByName("data.data", dir.objectId, datName);
            }
            catch(e){
                console.warn(e);
            }
            if(!img){
                ...
                img = await am7model._page.createObject(img);
            }

            char.profile.portrait = {id: img.id};
            await am7model._page.patchObject(char.profile);
        }
    ...
    function characterWizard(){
            ...
            let cfg = {
                ...
                confirm: async function (data) {
                    am7model._page.toast("info", "Generating ...");
                    let o = data.entity;
            ...
            am7model._page.components.dialog.setDialog(cfg);


    }

    async function rollCharacter(object, inst, gender) {
        let x = await m.request({ method: 'GET', url: am7model._client.base() + "/olio/roll" + (gender ? "/" + gender : ""), withCredentials: true });
        if (x && x != null) {
            if(object){
    ...
    async function getSystemPrompt(object, inst) {
        ...
        let x = await m.request({ method: 'GET', url: am7model._client.base() + "/chat/prompt", withCredentials: true });
        if (x && x != null) {
            object.mergeEntity(x);
        }
    }
    ...
    async function convertToPromptTemplate(objectPage, inst) {
        ...
        let cfg = inst.entity;
        if (!cfg.objectId) {
            am7model._page.toast("warn", "Save the prompt config before converting", 3000);
            return;
        }
        let sections = [];
        let order = [];
        ...
        try {
            let result = await new Promise(function(res) {
                am7model._client.create("olio.llm.promptTemplate", template, function(v) { res(v); });
            });
            if (result && result.objectId) {
                am7model._page.toast("success", "Created template: " + template.name, 5000);
            } else {
                am7model._page.toast("error", "Failed to create template", 5000);
            }
        } catch (e) {
            am7model._page.toast("error", "Conversion failed: " + (e.message || e), 5000);
        }
    }
