 ...
 *
 * Exports: CardGameVoice, getPlayerEmotion, getPlayerMoodColor,
 *          buildEmotionContext, getVoice, setVoice
 */
...
function showNarrationSubtitle(text) {
    if (typeof _gs?.showNarrationSubtitle === "function") {
        _gs.showNarrationSubtitle(text);
    } else {
...
class CardGameVoice {
    ...
    async initialize(voiceConfig) {
        ...
        if (typeof getPage()?.components?.audio?.createAudioSource !== "function") {
    ...
    async speak(text, options = {}) {
        ...
        try {
            ...
            const audioComponent = getPage()?.components?.audio;
...
function getPlayerEmotion() {
    if (!getPage()?.components?.moodRing?.enabled()) return null;
    return getPage().components.moodRing.emotion();
}

function getPlayerMoodColor() {
    if (!getPage()?.components?.moodRing?.enabled()) return null;
    return getPage().components.moodRing.moodColor();
}
