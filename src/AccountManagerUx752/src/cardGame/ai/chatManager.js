 ...
 *
 * Exports: CardGameChatManager, getChatManager, setChatManager
 */
...
- Negotiate: Be open or closed depending on your alignment`;
    ...
    async startConversation() {
        ...
        if (this.chatRequest && this.chatRequest.objectId && connector) {
            try {
                let hist = await connector.getHistory(this.chatRequest);
    ...
    async sendMessage(playerMessage, gameContext) {
        ...
        try {
            ...
            const contextMessages = this.currentConversation.slice(-4)
                .map(msg => `${msg.role === "player" ? this.playerName : this.npcName}: ${msg.text}`)
                .join("\n");
...
export { CardGameChatManager, getChatManager, setChatManager };
