 ...
 *
 * Ported from Ux7 IIFE (window.CardGame.Characters) to ESM.
 *
 * Dependencies:
 *   - gameConstants (DEFAULT_THEME, COMMON_ACTIONS, ACTION_DEFINITIONS)
 *   - ctx (shared mutable context: activeTheme, deckNameInput)
...
// ── Shared context accessors (set by CardGameApp via characters.wireContext) ──
let ctx = null;
let Constants = null;
let Engine = null;
...
async function resolveStatistics(charObj) {
    ...
    if (s.objectId && s.physicalStrength == null && s.agility == null) {
        try {
            ...
            let sqr = await getPage().search(sq);
...
function getPortraitUrl(char, size) {
    ...
    return g_application_path + "/thumbnail/" + getClient().dotPath(getClient().currentOrganization) +
...
function assembleCharacterCard(char) {
    ...
    let classActions = templateActions || (Constants && Constants.COMMON_ACTIONS) || [];
...
function assembleStarterDeck(chars, theme) {
    ...
    let ACTION_DEFINITIONS = (Constants && Constants.ACTION_DEFINITIONS) || {};
    let actionCardDefs = Object.entries(ACTION_DEFINITIONS).map(([name, def]) => ({
    ...
    let getThreatCreatures = Engine?.getThreatCreatures;
    ...
    let getScenarioCards = Engine?.getScenarioCards;
    if (getScenarioCards) {
...
async function loadAvailableCharacters() {
    ...
    try {
        ...
        if (deckNameInput) {
            try {
                let deckCharDir = await getPage().findObject("auth.group", "data", "~/CardGame/" + deckNameInput + "/Characters");
                if (deckCharDir) {
                    let q = am7view.viewQuery("olio.charPerson");
        ...
        getPage().toast("error", "Failed to load characters");
    }
    charsLoading = false;
    m.redraw();
}
...
async function generateCharactersFromTemplates(themeId, count) {
    ...
    if (!templates || !templates.templates) {
        getPage().toast('error', 'Character templates not available');
        return [];
    }
    ...
    let COMMON_ACTIONS = (Constants && Constants.COMMON_ACTIONS) || [];
    ...
    for (const template of selectedTemplates) {
        try {
            ...
            rolled._templateActions = template.actions || COMMON_ACTIONS;
    ...
    getPage().toast('success', `Generated ${generatedChars.length} characters`);
...
async function persistGeneratedCharacters(charList, deckName) {
    if (!charList || !charList.length) return [];

    let charDir;
    try {
        charDir = await getPage().makePath("auth.group", "data", "~/CardGame/" + deckName + "/Characters");
    ...
    for (const char of charList) {
        ...
        try {
            ...
            let existing = await getPage().searchFirst("olio.charPerson", charDir.id, char.name);
            if (existing) {
...
function charPersonToCard(charPerson) {
    ...
    return {
        ...
        portraitUrl: charPerson.profile?.portrait?.groupPath && charPerson.profile?.portrait?.name ?
            g_application_path + "/thumbnail/" + getClient().dotPath(getClient().currentOrganization) +
...
export const characters = {
    ...
    wireContext(nsCtx, nsConstants, nsEngine) {
        ctx = nsCtx;
...
console.log('[CardGame] Characters service loaded (ESM, ' +
    Object.keys(characters).length + ' exports)');
