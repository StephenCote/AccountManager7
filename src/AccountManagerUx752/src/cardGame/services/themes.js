 ...
 *
 * Ported from Ux7 IIFE (window.CardGame.Themes) to ESM.
 *
 * Dependencies:
 *   - gameConstants (THEME_DEFAULTS, DEFAULT_THEME)
 *   - storage (deckStorage, upsertDataRecord, loadDataRecord, listDataRecords, DECK_BASE_PATH)
 *   - characters (str, refreshCharacterCard) [loaded after this module]
 *   - am7model, am7model._page, am7model._client (late-bound)
 */
import m from 'mithril';
import { am7model } from '../../core/model.js';
import { am7view } from '../../core/view.js';

// Late-bound accessors
function getPage() { return am7model._page; }
function getClient() { return am7model._client; }

// ── Shared context accessors (set by CardGameApp via themes.wireContext) ──
let Constants = null;
let Storage = null;
let Characters = null;
let Engine = null;
...
async function loadThemeConfig(themeId) {
    ...
    try {
        ...
        if (resp && resp.themeId) {
            ...
            let THEME_DEFAULTS = (Constants && Constants.THEME_DEFAULTS) || {};
    ...
    let THEME_DEFAULTS = (Constants && Constants.THEME_DEFAULTS) || {};
    let DEFAULT_THEME = (Constants && Constants.DEFAULT_THEME) || {};
    ...
    if (td && id !== "high-fantasy") {
        activeTheme = {
            ...
            artStyle: {
                ...
                colorPalette: DEFAULT_THEME.artStyle ? DEFAULT_THEME.artStyle.colorPalette : undefined
            },
...
async function lookupColor(colorName) {
    ...
    let grp = await getPage().findObject("auth.group", "data", "/Library/Colors");
    if (!grp) {
...
async function applyThemeOutfits(deck, theme, skipSave) {
    ...
    if (!thm.outfits) {
        getPage().toast("warn", "Theme has no outfit definitions");
        return;
    }
    ...
    if (!charCards.length) {
        getPage().toast("warn", "No character cards with source IDs");
    ...
    let apparelDir = await getPage().findObject("auth.group", "data", "~/Apparel");
    if (!apparelDir) {
        getPage().toast("error", "Apparel directory not found at ~/Apparel");
        applyingOutfits = false;
        m.redraw();
        return;
    }
    let wearableDir = await getPage().findObject("auth.group", "data", "~/Wearables");
    if (!wearableDir) {
        getPage().toast("error", "Wearables directory not found at ~/Wearables");
        applyingOutfits = false;
        m.redraw();
        return;
    }
    ...
    for (let card of charCards) {
        try {
            getPage().toast("info", "Outfitting " + card.name + "...", -1);

            // Load full character with all nested store/apparel/wearable data
            getClient().clearCache("olio.charPerson");
            getClient().clearCache("olio.apparel");
            getClient().clearCache("olio.wearable");
            getClient().clearCache("olio.store");
            let char = await getClient().getFull("olio.charPerson", card.sourceId);
            if (!char) {
            ...
            let gender = (Characters ? Characters.str(char.gender) : String(char.gender || "")).toLowerCase();
            ...
            for (let ap of existingApparel) {
                ...
                if (ap.wearables && ap.wearables.length) {
                    let wPatches = ap.wearables.filter(w => w.inuse).map(w =>
                        getPage().patchObject({ schema: "olio.wearable", id: w.id, inuse: false })
                    );
                    if (wPatches.length) await Promise.all(wPatches);
                }
                // Deactivate apparel
                if (ap.inuse) {
                    await getPage().patchObject({ schema: "olio.apparel", id: ap.id, inuse: false });
                }
                // Remove old apparel from store membership so it won't interfere
                await new Promise(res => {
                    getClient().member("olio.store", storeObjId, "apparel", "olio.apparel", ap.objectId, false, res);
                });
            }

            // 2b. Clear existing items from store
            let storeItemList = await new Promise(res => {
                getClient().members("olio.store", storeObjId, "olio.item", 0, 50, res);
            });
            if (storeItemList && storeItemList.length) {
                for (let itemRef of storeItemList) {
                    await new Promise(res => {
                        getClient().member("olio.store", storeObjId, "items", "olio.item", itemRef.objectId, false, res);
                    });
                }
            }
            ...
            let storeInvList = await new Promise(res => {
                getClient().members("olio.store", storeObjId, "olio.inventoryEntry", 0, 50, res);
            });
            if (storeInvList && storeInvList.length) {
                for (let invRef of storeInvList) {
                    await new Promise(res => {
                        getClient().member("olio.store", storeObjId, "inventory", "olio.inventoryEntry", invRef.objectId, false, res);
                    });
                }
            }
            ...
            let createdApparel = await getPage().createObject(apparel);
            if (!createdApparel) {
            ...
            for (let wDef of outfitDef.wearables) {
                ...
                let created = await getPage().createObject(wearable);
            ...
            await new Promise(res => {
                getClient().member("olio.store", storeObjId, "apparel", "olio.apparel", createdApparel.objectId, true, res);
            });

            // 7. Patch inuse = true on all wearables and the apparel
            let inusePatches = createdWearables.map(cw =>
                getPage().patchObject({ schema: "olio.wearable", id: cw.id, inuse: true })
            );
            inusePatches.push(
                getPage().patchObject({ schema: "olio.apparel", id: createdApparel.id, inuse: true })
            );
            ...
            await m.request({ method: 'GET', url: getClient().base() + "/olio/olio.charPerson/" + card.sourceId + "/narrate", withCredentials: true });

            // 10. Refresh character card data
            if (Characters && Characters.refreshCharacterCard) {
                await Characters.refreshCharacterCard(card);
            }
            processed++;
            ...
            getPage().clearToast();  // Clear indefinite progress toast
            getPage().toast("error", "Failed to outfit " + card.name);
        }
    }
    ...
    if (processed > 0) {
        if (!skipSave && Storage) {
            ...
            await Storage.deckStorage.save(safeName, deck);
        }
        getPage().toast("success", "Applied outfits to " + processed + " character(s)");
    }
    m.redraw();
}
...
function getThemeStoragePath() {
    return (Storage && Storage.DECK_BASE_PATH ? Storage.DECK_BASE_PATH : "~/CardGame") + "/themes";
}
...
const themeStorage = {
    async save(themeId, themeData) {
        try {
            ...
            return await Storage.upsertDataRecord(getThemeStoragePath(), themeId + ".json", themeData);
        } catch (e) {
    ...
    async load(themeId) {
        try {
            if (!Storage) return null;
            return await Storage.loadDataRecord(getThemeStoragePath(), themeId + ".json", false);
        } catch (e) {
            return null;
        }
    },
    async list() {
        try {
            if (!Storage) return [];
            let records = await Storage.listDataRecords(getThemeStoragePath());
    ...
    async remove(themeId) {
        try {
            let grp = await getPage().findObject("auth.group", "DATA", getThemeStoragePath());
            if (!grp) return;
            ...
            if (qr?.results?.length) {
                await getPage().deleteObject("data.data", qr.results[0].objectId);
            }
        } catch (e) {
                        ...
                        m("button", {
                            ...
                            async onclick() {
                                try {
                                    ...
                                    themeEditorState.dirty = false;
                                    getPage().toast("success", "Theme saved: " + parsed.name);
                                    loadThemeList();
                                } catch (e) {
                                    getPage().toast("error", "Save failed: " + e.message);
                                }
                            }
                        }, "Save Theme")
                    ]),
                        ...
                        themeEditorState.effectPreview ? m("div", { class: "cg2-effect-preview" }, [
                            m("div", { style: { display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" } }, [
                                ...
                                m("button", { class: "cg2-btn cg2-btn-sm", onclick() {
                                    ...
                                    else getPage().toast("info", themeEditorState.effectPreview.trim());
                                } }, "Copy"),
                            ...
                            m("div", { style: { fontSize: "10px", marginTop: "4px" } }, (() => {
                                let parseEffect = Engine?.parseEffect;
                                if (!parseEffect) return m("span", { style: { color: "#888" } }, "(Engine not loaded)");
...
STATUS EFFECTS: stunned(1t), poisoned(3t, -2HP/t), burning(2t, -3HP/t),
  ...
  inspired(2t, +2 rolls), regenerating(3t, +2HP/t)`)
            ...
            return m("div", { class: "cg2-theme-editor" }, [
                m("div", { class: "cg2-toolbar" }, [
                    m("button", { class: "cg2-btn", onclick() { themes.navigateBack(); } }, "\u2190 Back"),
                    m("span", { style: { fontWeight: 700, fontSize: "16px", marginLeft: "8px" } }, "Theme Editor"),
                    m("button", {
                        class: "cg2-btn cg2-btn-primary", style: { marginLeft: "auto" },
...
// ── Public API ──────────────────────────────────────────────────
export const themes = {
    // Wire shared context references (called by CardGameApp)
    wireContext(nsConstants, nsStorage, nsCharacters, nsEngine) {
        Constants = nsConstants;
        Storage = nsStorage;
        Characters = nsCharacters;
        Engine = nsEngine;
        // Initialize activeTheme from Constants if not yet set
        if (!activeTheme && Constants && Constants.DEFAULT_THEME) {
            activeTheme = Constants.DEFAULT_THEME;
        }
    },
    state: {
...
console.log('[CardGame] Themes service loaded (ESM)');
