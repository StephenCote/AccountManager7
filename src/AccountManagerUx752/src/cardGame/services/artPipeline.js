 ...
 *
 * Ported from Ux7 IIFE (window.CardGame.ArtPipeline) to ESM.
 *
 * Dependencies:
 *   - gameConstants (DEFAULT_THEME)
 *   - ctx (shared mutable context: viewingDeck, gameState, activeTheme)
 *   - storage (deckStorage)
 *   - characters (refreshCharacterCard)
 *   - am7model._sd (SD config)
 *   - am7model._olio (Olio API)
 *   - am7model._page, am7model._client (late-bound)
 */
...
function getSd() { return am7model._sd; }
function getOlio() { return am7model._olio; }
function getAppPath() { return getClient().base().replace("/rest", ""); }

const DECK_BASE_PATH = "~/CardGame";

// ── Shared context accessors (set by CardGameApp via artPipeline.ctx) ──
let ctx = null;
let Constants = null;
let Storage = null;
let Characters = null;
...
function getActiveTheme() {
    return (ctx && ctx.activeTheme) || (Constants && Constants.DEFAULT_THEME) || {};
}
function getViewingDeck() {
    return ctx ? ctx.viewingDeck : null;
}
function getDeckStorage() {
    return Storage ? Storage.deckStorage : null;
}
function getGameState() {
    return ctx ? ctx.gameState : null;
}
...
function getSdOverrideInst(key) {
    if (!sdOverrideInsts[key]) {
        ...
        sdOverrideViews[key] = getPage().views.object();
    }
...
async function ensureArtDir(themeId) {
    ...
    artDir = await getPage().makePath("auth.group", "DATA", path);
    return artDir;
}
...
async function generateBackground(theme) {
    ...
    try {
        ...
        let result = await m.request({
            method: "POST",
            url: getAppPath() + "/rest/olio/generateArt",
            body: entity,
        ...
        backgroundThumbUrl = getAppPath() + "/thumbnail/" +
            getClient().dotPath(getClient().currentOrganization) +
        ...
        getPage().toast("success", "Background generated");
        ...
        getPage().toast("error", "Background failed: " + (e.message || "Unknown error"));
    }
...
async function generateTabletop(theme) {
    ...
    try {
        ...
        tabletopThumbUrl = getAppPath() + "/thumbnail/" +
            getClient().dotPath(getClient().currentOrganization) +
            "/data.data" + (result.groupPath || dir.path) + "/" + result.name + "/256x256";
        ...
        getPage().toast("success", "Tabletop generated");
        ...
        getPage().toast("error", "Tabletop failed: " + (e.message || "Unknown error"));
    }
...
async function generateCardArt(card, theme) {
    ...
    if (card.type === "character" && card.sourceId) {
        ...
        if (gp && rname) {
            let orgPath = getClient().dotPath(getClient().currentOrganization);
    ...
    let result = await m.request({
        method: "POST",
        url: g_application_path2 + "/rest/olio/generateArt",
        body: sdEntity,
        withCredentials: true,
        extract: function(xhr) {
            let body = null;
    ...
    if (result.groupId && dir.id && result.groupId !== dir.id) {
        await getPage().moveObject(result, dir);
...
async function generateTemplateArt(side) {
    ...
    try {
        ...
        getPage().toast("success", "Card " + side + " art generated");
    } catch (e) {
        console.error("[CardGame ArtPipeline] Template art failed:", e);
        getPage().toast("error", "Card " + side + " art failed: " + (e.message || "Unknown error"));
    }
...
async function processArtQueue() {
    ...
    if (!next) {
        artProcessing = false;
        if (artCompleted > 0) {
            getPage().toast("success", "Art generation complete: " + artCompleted + " of " + artTotal + " images");
        ...
        if (viewingDeck && viewingDeck.cards && viewingDeck.cards[next.cardIndex]) {
            ...
            if (result.thumbUrl) {
                ...
                if (Characters && Characters.refreshCharacterCard) {
                    await Characters.refreshCharacterCard(deckCard);
                }
            }
...
async function queueDeckArt(deck) {
    ...
    if (artTotal === 0) {
        getPage().toast("info", "All cards already have art");
        return;
    }
...
async function pauseArtQueue() {
    ...
    getPage().toast("info", "Art generation paused. Progress saved (" + artCompleted + " of " + artTotal + ")");
    m.redraw();
}
...
async function cancelArtQueue() {
    ...
    await saveArtProgress();
    if (artCompleted > 0) {
        getPage().toast("info", "Art generation cancelled. Progress saved (" + artCompleted + " of " + artTotal + ")");
    }
    m.redraw();
}
...
async function generateImageSequence(card, options) {
    ...
    if (!card.sourceId && card._sourceChar) {
        getPage().toast("info", "Save deck first to generate images for generated characters");
        return;
    }
    if (!card.sourceId) return;
    sequenceCardId = card.sourceId;
    sequenceProgress = "Loading character...";
    getPage().toast("info", "Image Sequence: Loading character...", -1);
    m.redraw();

    try {
        // Load character to get store reference
        getClient().clearCache("olio.charPerson");
        let char = await getPage().searchFirst("olio.charPerson", undefined, undefined, card.sourceId);
        ...
        await getClient().clearCache("olio.store");
        let sto = await getClient().getFull("olio.store", char.store.objectId);
        ...
        await getClient().clearCache("olio.apparel");
        let fullApparel = await getClient().getFull("olio.apparel", activeApparel.objectId);
        ...
        getPage().toast("info", "Image Sequence: Dressing down...", -1);
        m.redraw();
        ...
        getPage().toast("info", "Image Sequence: Generating 1 of " + totalImages + " (NONE)...", -1);
        m.redraw();
        ...
        if (x) {
            ...
            await getClient().patchAttribute(x, "wearLevel", "NONE/0");
            images.push(x);
        }
        ...
        for (let i = 0; i < lvls.length; i++) {
            ...
            getPage().toast("info", "Image Sequence: Generating " + (images.length + 1) + " of " + totalImages + " (" + lvls[i] + ")...", -1);
            m.redraw();
            ...
            if (!x) {
                getPage().toast("error", "Failed to create image at level " + lvls[i]);
                break;
            }

            // Tag image with wear level
            let levelIndex = am7model.enums.wearLevelEnumType.findIndex(we => we == lvls[i]);
            await getClient().patchAttribute(x, "wearLevel", lvls[i] + "/" + levelIndex);
        ...
        if (images.length > 0) {
            getPage().toast("success", "Created " + images.length + " images for " + card.name);
            ...
            if (!options?.skipGallery) {
                let openGallery = Rendering?.openGalleryPicker;
                if (openGallery) {
                    openGallery(card);
                }
            }
        } else {
            getPage().toast("warn", "No images were generated");
        }

    } catch (e) {
        console.error("[CardGame ArtPipeline] Image sequence failed:", e);
        getPage().clearToast();  // Clear indefinite progress toast
        getPage().toast("error", "Image sequence failed: " + e.message);
        sequenceProgress = "";
        sequenceCardId = null;
        m.redraw();
    }
...
async function generateAllSequences() {
    ...
    if (batchSequenceActive || sequenceCardId) {
        getPage().toast("warn", "A sequence generation is already in progress");
        return;
    }
    ...
    if (!charCards.length) {
        getPage().toast("warn", "No saved character cards found in deck");
        return;
    }
    ...
    getPage().toast("info", "Generating sequences for " + charCards.length + " character(s)...");
    ...
    batchSequenceCurrentName = "";
    m.redraw();
    getPage().toast("success", "All character sequences complete (" + batchSequenceCompleted + "/" + batchSequenceTotal + ")");
}
...
export const artPipeline = {
    ...
    wireContext(nsCtx, nsConstants, nsStorage, nsCharacters, nsEngine, nsRendering) {
        ctx = nsCtx;
        Constants = nsConstants;
        Storage = nsStorage;
        Characters = nsCharacters;
        Engine = nsEngine;
        Rendering = nsRendering;
    },
    state: {
...
console.log('[CardGame] ArtPipeline service loaded (ESM, ' +
    Object.keys(artPipeline).length + ' exports)');
