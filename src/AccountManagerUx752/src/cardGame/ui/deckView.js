/**
 * CardGame UI - Deck View / Editor with Art Generation Controls (ESM)
 * Card grid display with flip capability, art generation controls, SD Config panel,
 * Game Config panel (LLM, voice settings), and refresh/outfit controls.
...
function checkForSaves(viewingDeck) {
    ...
    if (NS?.Storage && NS.Storage.gameStorage) {
        NS.Storage.gameStorage.list(deckId).then(function(saves) {
        ...
        console.warn("[CardGame v2] Save check: NS.Storage.gameStorage not available");
        saveCheckPending = false;
    }
}
...
function DeckView() {
    return {
        oninit() {
            let ctxObj = NS?.ctx || {};
            let viewingDeck = ctxObj.viewingDeck;
            ...
            if (viewingDeck && viewingDeck.backgroundImageId) {
                ctxObj.backgroundImageId = viewingDeck.backgroundImageId;
                ctxObj.backgroundPrompt = viewingDeck.backgroundPrompt || null;
                ctxObj.backgroundThumbUrl = viewingDeck.backgroundThumbUrl || null;
            } else {
                ctxObj.backgroundImageId = null;
                ctxObj.backgroundPrompt = null;
                ctxObj.backgroundThumbUrl = null;
            }
            if (viewingDeck && viewingDeck.tabletopImageId) {
                ctxObj.tabletopImageId = viewingDeck.tabletopImageId;
                ctxObj.tabletopThumbUrl = viewingDeck.tabletopThumbUrl || null;
            } else {
                ctxObj.tabletopImageId = null;
                ctxObj.tabletopThumbUrl = null;
            }
            if (viewingDeck && viewingDeck.sdOverrides) {
                ...
                ctxObj.sdOverrides = NS.ArtPipeline.defaultSdOverrides();
            }
            NS.ArtPipeline.resetSdOverrideInsts();
            ctxObj.sdConfigExpanded = false;
            ctxObj.sdConfigTab = "_default";
            ctxObj.gameConfigExpanded = false;
            ctxObj.flippedCards = {};
        },
        view() {
            let ctxObj = NS?.ctx || {};
            let viewingDeck = ctxObj.viewingDeck;
            let activeTheme = ctxObj.activeTheme;
            ...
            let artQueue = ctxObj.artQueue || [];
            let backgroundGenerating = ctxObj.backgroundGenerating;
            let tabletopGenerating = ctxObj.tabletopGenerating;
            let cardFrontGenerating = ctxObj.cardFrontGenerating;
            let cardBackGenerating = ctxObj.cardBackGenerating;
            let sequenceCardId = ctxObj.sequenceCardId;
            let backgroundImageId = ctxObj.backgroundImageId;
            let backgroundThumbUrl = ctxObj.backgroundThumbUrl;
            let tabletopImageId = ctxObj.tabletopImageId;
            let tabletopThumbUrl = ctxObj.tabletopThumbUrl;
            let cardFrontImageUrl = ctxObj.cardFrontImageUrl;
            let cardBackImageUrl = ctxObj.cardBackImageUrl;
            let flippedCards = ctxObj.flippedCards || {};
            let sdConfigExpanded = ctxObj.sdConfigExpanded;
            let sdConfigTab = ctxObj.sdConfigTab || "_default";
            let gameConfigExpanded = ctxObj.gameConfigExpanded;
            let voiceProfiles = ctxObj.voiceProfiles || [];
            let voiceProfilesLoaded = ctxObj.voiceProfilesLoaded;
            let applyingOutfits = ctxObj.applyingOutfits;
            ...
            return m("div", [
                m("div", { class: "cg2-toolbar" }, [
                    m("button", { class: "cg2-btn", onclick: () => { ctxObj.screen = "deckList"; ctxObj.viewingDeck = null; NS.ArtPipeline.cancelArtQueue(); ctxObj.artTotal = 0; ctxObj.backgroundImageId = null; ctxObj.backgroundThumbUrl = null; NS.UI.loadSavedDecks(); m.redraw(); } }, "\u2190 Back to Decks"),
                    ...
                    m("button", {
                        ...
                        async onclick() {
                            await NS.Characters.refreshAllCharacters(viewingDeck);
                            getPage().toast("success", "Character data refreshed");
                            m.redraw();
                        }
                    }, [
                    ...
                    m("button", {
                        ...
                        async onclick() {
                            await NS.Themes.applyThemeOutfits(viewingDeck, activeTheme);
                        }
                    }, [
                    ...
                    deckHasSave ? m("button", {
                        ...
                        onclick() {
                            if (ctxObj.resumeGame) ctxObj.resumeGame(viewingDeck.storageName);
                        }
                    }, [
                    ...
                    m("button", {
                        ...
                        onclick() {
                            ctxObj.gameState = null;
                            ctxObj.gameCharSelection = null;
                            ctxObj.screen = "game";
                            m.redraw();
                        }
                    }, [
                    ...
                    m("button", {
                        ...
                        onclick() {
                            ...
                            if (NS.TestMode && NS.TestMode.setTestDeck) {
                                NS.TestMode.setTestDeck(viewingDeck, deckName);
                            }
                            ctxObj.screen = "test";
                            m.redraw();
                        }
                    ...
                    m("button", {
                        ...
                        onclick() {
                            ctxObj.designerDeck = viewingDeck;
                    ...
                    m("button", {
                        ...
                        onclick() {
                            if (NS.Designer && NS.Designer.ExportDialog) {
                                NS.Designer.ExportDialog.open(viewingDeck);
                            }
                        }
                    }, [
                ...
                m("div", { class: "cg2-sd-panel" }, [
                    m("div", {
                        ...
                        onclick() { ctxObj.sdConfigExpanded = !ctxObj.sdConfigExpanded; m.redraw(); }
                    }, [
                        m("span", { class: "material-symbols-outlined", style: { fontSize: "16px", marginRight: "6px", transition: "transform 0.2s", transform: ctxObj.sdConfigExpanded ? "rotate(90deg)" : "" } }, "chevron_right"),
                        m("span", { style: { fontWeight: 600, fontSize: "13px" } }, "SD Config"),
                        (() => {
                            let insts = ctxObj.sdOverrideInsts || {};
                    ...
                    ctxObj.sdConfigExpanded ? m("div", { class: "cg2-sd-panel-body" }, [
                        // Type tabs
                        m("div", { class: "cg2-sd-tabs" }, [
                            m("span", {
                                class: "cg2-sd-tab" + (ctxObj.sdConfigTab === "_default" ? " cg2-sd-tab-active" : ""),
                                onclick() { ctxObj.sdConfigTab = "_default"; m.redraw(); }
                            }, "Default"),
                            ...Object.keys(CARD_TYPES).map(t =>
                                m("span", {
                                    class: "cg2-sd-tab" + (ctxObj.sdConfigTab === t ? " cg2-sd-tab-active" : ""),
                                    style: { borderBottomColor: ctxObj.sdConfigTab === t ? CARD_TYPES[t].color : "transparent" },
                                    onclick() { ctxObj.sdConfigTab = t; m.redraw(); }
                                }, [
                            ...
                            ...Object.keys(TEMPLATE_TYPES).map(t =>
                                m("span", {
                                    class: "cg2-sd-tab" + (ctxObj.sdConfigTab === t ? " cg2-sd-tab-active" : ""),
                                    style: { borderBottomColor: ctxObj.sdConfigTab === t ? TEMPLATE_TYPES[t].color : "transparent" },
                                    onclick() { ctxObj.sdConfigTab = t; m.redraw(); }
                                }, [
                        ...
                        (() => {
                            let inst = NS.ArtPipeline.getSdOverrideInst(ctxObj.sdConfigTab);
                            let sdOvViews = ctxObj.sdOverrideViews || {};
                            let ov = sdOvViews[ctxObj.sdConfigTab];
                            return m(ov.view, {
                                freeForm: true,
                        ...
                        m("div", { style: { display: "flex", justifyContent: "space-between", marginTop: "6px" } }, [
                            ...
                            m("div", { style: { display: "flex", gap: "6px" } }, [
                                m("button", {
                                    class: "cg2-btn", style: { fontSize: "11px" },
                                    onclick() {
                                        if (ctxObj.sdConfigTab === "_default") {
                                            ctxObj.sdOverrides[ctxObj.sdConfigTab] = NS.ArtPipeline.newSdOverride();
                                        } else {
                                            let copy = JSON.parse(JSON.stringify(ctxObj.sdOverrides._default));
                                            copy[am7model.jsonModelKey] = "olio.sd.config";
                                            ctxObj.sdOverrides[ctxObj.sdConfigTab] = copy;
                                        }
                                        let insts = ctxObj.sdOverrideInsts || {};
                                        let views = ctxObj.sdOverrideViews || {};
                                        delete insts[ctxObj.sdConfigTab];
                                        delete views[ctxObj.sdConfigTab];
                                        m.redraw();
                                    }
                                }, "Clear " + (ctxObj.sdConfigTab === "_default" ? "Default" : (CARD_TYPES[ctxObj.sdConfigTab] || TEMPLATE_TYPES[ctxObj.sdConfigTab] || {}).label || ctxObj.sdConfigTab)),
                                m("button", {
                                    class: "cg2-btn cg2-btn-primary", style: { fontSize: "11px" },
                                    async onclick() {
                                        if (viewingDeck) {
                                            let allKeys = Object.keys(CARD_TYPES).concat(Object.keys(TEMPLATE_TYPES));
                                            let overrides = ctxObj.sdOverrides || {};
                                            allKeys.forEach(k => {
                                                if (!overrides[k]) {
                ...
                m("div", { class: "cg2-sd-panel" }, [
                    m("div", {
                        ...
                        m("span", { class: "material-symbols-outlined", style: { fontSize: "16px", marginRight: "6px", transition: "transform 0.2s", transform: ctxObj.gameConfigExpanded ? "rotate(90deg)" : "" } }, "chevron_right"),
                        m("span", { style: { fontWeight: 600, fontSize: "13px" } }, "Game Config"),
                        (() => {
                            let gc = NS.ArtPipeline.getDeckGameConfig(viewingDeck);
                            let tags = [];
                    ...
                    ctxObj.gameConfigExpanded ? m("div", { style: { padding: "12px 16px" } }, (() => {
                        let gc = NS.ArtPipeline.getDeckGameConfig(viewingDeck);
                        let vProfiles = ctxObj.voiceProfiles || [];
                        let vLoaded = ctxObj.voiceProfilesLoaded;
                                ...
                                m("div", { class: "cg2-config-control" }, [
                                    m("span", { style: { fontSize: "12px", color: "#666" } },
                                        (function() {
                                            ...
                                            let profiles = NS?.AI?.CardGameNarrator?.PROFILES;
                                ...
                                m("div", { class: "cg2-config-control" }, [
                                    m("button", {
                                        ...
                                        async onclick() {
                                            ...
                                            if (!safeName) return;
                                            let count = await NS.UI.deleteGameChats(safeName);
                                            if (count > 0) {
                                                getPage().toast("success", "Deleted " + count + " game chat(s)");
                                            } else {
                                                getPage().toast("info", "No game chats found for this deck");
                                            }
                                            m.redraw();
                                        }
                                    }, [
                    ...
                    m("div", { class: "cg2-bg-panel", style: { flex: "1 1 200px" } }, [
                        ...
                        m("div", { class: "cg2-bg-panel-right" }, [
                            m("button", {
                                class: "cg2-btn",
                                disabled: busy,
                                onclick() { NS.ArtPipeline.generateBackground(activeTheme); }
                            }, [
                            ...
                            backgroundImageId ? m("button", {
                                class: "cg2-btn",
                                disabled: busy,
                                onclick() { ctxObj.backgroundImageId = null; ctxObj.backgroundThumbUrl = null; m.redraw(); }
                            }, "\u00d7") : null
                        ])
                    ]),
                    m("div", { class: "cg2-bg-panel", style: { flex: "1 1 200px" } }, [
                        ...
                        m("div", { class: "cg2-bg-panel-right" }, [
                            m("button", {
                                class: "cg2-btn",
                                disabled: busy,
                                onclick() { NS.ArtPipeline.generateTabletop(activeTheme); }
                            }, [
                                tabletopGenerating
                                    ? m("span", { class: "material-symbols-outlined cg2-spin", style: { fontSize: "14px", verticalAlign: "middle", marginRight: "4px" } }, "progress_activity")
                            ...
                            tabletopImageId ? m("button", {
                                class: "cg2-btn",
                                disabled: busy,
                                onclick() { ctxObj.tabletopImageId = null; ctxObj.tabletopThumbUrl = null; m.redraw(); }
                            }, "\u00d7") : null
                        ])
                    ]),
                    (function() {
                        ...
                        let batchActive = NS.ArtPipeline.state.batchSequenceActive;
                        let batchDone = NS.ArtPipeline.state.batchSequenceCompleted;
                        let batchTotal = NS.ArtPipeline.state.batchSequenceTotal;
                        return m("div", { class: "cg2-bg-panel", style: { flex: "1 1 200px" } }, [
                            ...
                            m("div", { class: "cg2-bg-panel-right" }, [
                                m("button", {
                                    ...
                                    onclick() { NS.ArtPipeline.generateAllSequences(); }
                                }, [
                                    batchActive
                                        ? m("span", { class: "material-symbols-outlined cg2-spin", style: { fontSize: "14px", verticalAlign: "middle", marginRight: "4px" } }, "progress_activity")
                ...
                m("div", { class: "cg2-toolbar" }, [
                    m("button", {
                        ...
                        onclick() { NS.ArtPipeline.queueDeckArt(viewingDeck); }
                    }, [
                    ...
                    m("button", {
                        ...
                        onclick() {
                            ...
                            ctxObj.cardFrontImageUrl = null;
                            ctxObj.cardBackImageUrl = null;
                            ctxObj.backgroundImageId = null;
                            ctxObj.backgroundThumbUrl = null;
                            ctxObj.tabletopImageId = null;
                            ctxObj.tabletopThumbUrl = null;
                            ...
                            NS.ArtPipeline.queueDeckArt(viewingDeck);
                        }
                    }, [
                ...
                m(NS.ArtPipeline.ArtQueueProgress),
                // Card grid
                    ...
                    m("div", { class: "cg2-card-art-wrapper", key: "card-front" }, [
                        ...
                        !busy && !cardFrontGenerating ? m("div", { class: "cg2-card-actions" }, [
                            m("button", {
                                ...
                                onclick(e) { e.stopPropagation(); NS.ArtPipeline.generateTemplateArt("front"); }
                    ...
                    m("div", { class: "cg2-card-art-wrapper", key: "card-back" }, [
                        ...
                        !busy && !cardBackGenerating ? m("div", { class: "cg2-card-actions" }, [
                            m("button", {
                                ...
                                onclick(e) { e.stopPropagation(); NS.ArtPipeline.generateTemplateArt("back"); }
                            }, m("span", { class: "material-symbols-outlined", style: { fontSize: "14px" } }, "auto_awesome"))
                        ]) : null
                    ]),
                    ...
                    ...uniqueCards.map(({ card, index: i, count }) => {
                        ...
                        return m("div", { class: "cg2-card-art-wrapper", key: card.name + "-" + i }, [
                            ...
                            !busy ? m("div", { class: "cg2-card-actions" }, [
                                m("button", {
                                    class: "cg2-card-action-btn",
                                    title: "Regenerate art\n" + NS.ArtPipeline.buildCardPrompt(card, activeTheme),
                                    onclick(e) {
                                        e.stopPropagation();
                                        delete card.imageUrl;
                                        delete card.artObjectId;
                                        if (card.type === "character") delete card.portraitUrl;
                                        ctxObj.artQueue = [{ card, cardIndex: i, status: "pending", result: null, error: null }];
                                        ctxObj.artCompleted = 0;
                                        ctxObj.artTotal = 1;
                                        ctxObj.artPaused = false;
                                        ctxObj.artDir = null;
                                        NS.ArtPipeline.processArtQueue();
                                    }
                                }, m("span", { class: "material-symbols-outlined", style: { fontSize: "14px" } }, "auto_awesome")),
                                card.sourceId && card.type === "character" ? m("button", {
                                    ...
                                    onclick(e) {
                                        e.stopPropagation();
                                        NS.Rendering.openGalleryPicker(card);
                                    }
                                ...
                                card.sourceId && card.type === "character" ? (function() {
                                    ...
                                    let seqProgress = ctxObj.sequenceProgress || "";
                                    return m("button", {
                                        class: "cg2-card-action-btn",
                                        ...
                                        onclick(e) {
                                            e.stopPropagation();
                                            NS.ArtPipeline.generateImageSequence(card);
                                        }
                                ...
                                card.sourceId && card.type === "character" ? m("button", {
                                    ...
                                    async onclick(e) {
                                        e.stopPropagation();
                                        let ok = await NS.Characters.refreshCharacterCard(card);
                                        if (ok) {
                                            getPage().toast("success", "Refreshed: " + card.name);
                                        } else {
                                            getPage().toast("warn", "Could not refresh " + card.name);
                                        }
                                        m.redraw();
                                    }
                ...
                NS.Designer && NS.Designer.ExportDialog
                    ? m(NS.Designer.ExportDialog.ExportDialogOverlay, { deck: viewingDeck })
                    : null,
...
function PrintRulesSheet() {
    return {
        view(vnode) {
            ...
            let AD = NS?.Constants?.ACTION_DEFINITIONS || {};
            let commonActions = NS?.Constants?.COMMON_ACTIONS || [];
