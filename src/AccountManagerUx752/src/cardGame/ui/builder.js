/**
 * CardGame UI - Builder Steps (ESM)
 * Theme selection, character picker, card editor, and review/build steps.
 * Ported from Ux7 IIFE to ESM.
 *
 * Dependencies:
 *   - gameConstants (CARD_TYPES)
 *   - themes (loadThemeConfig, applyThemeOutfits, state.activeTheme, state.applyingOutfits)
 *   - characters (state.*, getPortraitUrl, mapStats, getCharId, clampStat, str,
...
// These will be resolved at runtime from the NS passed via init()
let NS = null;
...
function C() { return NS?.Constants || {}; }

// ── State ──────────────────────────────────────────────────────────
let editingCard = null;
let editingCardIndex = -1;
let addCardTypeOpen = false;
    ...
    return m("div", {
        ...
        onclick() {
            if (isSelected) {
                ...
                getPage().toast("warn", "Maximum 8 characters per deck");
            }
            m.redraw();
        }
    }, [
