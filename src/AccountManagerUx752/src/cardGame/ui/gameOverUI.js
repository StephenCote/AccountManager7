/**
 * CardGame UI - Game Over (ESM)
 * Victory/defeat screen with stats, campaign record, and level-up stat picker.
    ...
    return {
        async oninit() {
            ...
            if (!narratedEnd && gameState && gameState.winner) {
                narratedEnd = true;
                NS.GameState.narrateGameEnd(gameState.winner);
            }
            ...
            if (!campaignSaved && gameState && gameState.winner) {
                ...
                ctxObj.activeCampaign = await NS.Storage.saveCampaignProgress(gameState, isVictory);
                ...
                NS.Storage.gameStorage.deleteAll(gameState.deckName);
                m.redraw();
            }
        },
        view() {
            let ctxObj = NS?.ctx || {};
            let gameState = ctxObj.gameState;
            let activeCampaign = ctxObj.activeCampaign;
                    ...
                    m("div", { class: "cg2-game-over-actions" }, [
                        m("button", {
                            ...
                            async onclick() {
                                ...
                                if (NS.GameState?.cleanupAudio) NS.GameState.cleanupAudio();
                                let viewingDeck = ctxObj.viewingDeck;
                                // Restart with same characters
                                let playerChar = player.character;
                                ctxObj.gameState = await NS.GameState.createGameState(viewingDeck, playerChar);
                                if (ctxObj.gameState) {
                                    NS.GameState.initializeLLMComponents(ctxObj.gameState, viewingDeck);
                                    NS.GameState.resetInitAnimState();
                                    NS.GameState.startInitiativeAnimation();
                                }
                                m.redraw();
                            }
                        }, "Play Again"),
...
// ── Level Up UI ──────────────────────────────────────────────────
function LevelUpUI() {
    let STAT_DESCRIPTIONS = NS?.Constants?.STAT_DESCRIPTIONS || {};

    return {
        view() {
            let ctxObj = NS?.ctx || {};
            let gameState = ctxObj.gameState;
            let levelUpState = ctxObj.levelUpState;
                    ...
                    m("div", { class: "cg2-levelup-actions" }, [
                        m("button", {
                            ...
                            async onclick() {
                                ...
                                let gs = ctxObj.gameState;
                                await NS.Storage.campaignStorage.save(gs ? gs.deckName : "", campaign);
                                ctxObj.activeCampaign = campaign;
