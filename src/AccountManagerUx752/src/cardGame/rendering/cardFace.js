 ...
 *
 * Depends on: gameConstants, cardComponents, overlays (showImagePreview)
 */
import m from 'mithril';
import { gameConstants as C } from '../constants/gameConstants.js';
import { rarityStars, NeedBar, cornerIcon, iconDetail } from './cardComponents.js';
import { showImagePreview } from './overlays.js';

// Late-bound: Designer modules are loaded by CardGameApp and may not be available at import time.
// We access them through a lazy getter that checks the barrel NS when needed.
let _designerRef = null;
export function _setDesignerRef(ref) { _designerRef = ref; }
function getDesigner() { return _designerRef; }

// Late-bound: cardGameCtx may be wired after import
let _ctxFn = null;
...
function renderCharacterBody(card, options) {
    ...
    return [
        ...
        m("div", { class: "cg2-card-details" }, [
            ...
            // 3 need bars
            m(NeedBar, { label: "Health", abbrev: "HP", current: needs.hp != null ? needs.hp : 20, max: 20, color: "#e53935" }),
            m(NeedBar, { label: "Energy", abbrev: "NRG", current: needs.energy != null ? needs.energy : 14, max: stats.MAG || 14, color: "#1E88E5" }),
            m(NeedBar, { label: "Morale", abbrev: "MRL", current: needs.morale != null ? needs.morale : 20, max: 20, color: "#43A047" })
        ])
    ];
}
...
function renderCharacterBackBody(card, bgImage) {
    ...
    return m("div", { class: "cg2-card cg2-card-front cg2-char-back", style: backStyle }, [
        ...
        m("div", { class: "cg2-card-body" }, [
            // Alignment
            card.alignment ? iconDetail("balance", card.alignment) : null,
            ...
            (card.height || card.bodyType || card.bodyShape) ? m("div", { class: "cg2-char-back-body-info" }, [
                card.height ? (function() {
                    ...
                    return iconDetail("straighten", htStr + (wtStr ? ", " + wtStr : ""));
                })() : null,
                card.bodyType ? iconDetail("fitness_center", card.bodyType.toLowerCase()) : null,
                card.bodyShape ? iconDetail("accessibility_new", card.bodyShape.replace(/_/g, " ").toLowerCase()) : null
            ]) : null,
            // Description
...
function renderScenarioBody(card, options) {
    ...
    return [
        ...
        m("div", { class: "cg2-card-details" }, [
            ...
            card.description ? iconDetail("info", card.description) : null,
            card.bonus === "heal" ? iconDetail("favorite", "+" + (card.bonusAmount || 3) + " HP to all") : null,
            card.bonus === "energy" ? iconDetail("bolt", "+" + (card.bonusAmount || 2) + " Energy to all") : null,
            card.bonus === "loot" ? iconDetail("inventory_2", "Discover loot!") : null,
            card.threatBonus != null && card.effect === "threat" ? iconDetail("warning", "Threat DC +" + card.threatBonus) : null
        ])
    ];
}
...
function renderLootBody(card, options) {
    ...
    return [
        ...
        m("div", { class: "cg2-card-details" }, [
            card.source ? iconDetail("pets", "From: " + card.source) : null,
            card.rarity ? m("div", { class: "cg2-card-detail cg2-icon-detail" }, [
                m("span", { class: "material-symbols-outlined cg2-detail-icon" }, "star"),
                m("span", rarityStars(card.rarity))
            ]) : null,
            card.effect ? iconDetail("auto_awesome", card.effect) : null
        ])
    ];
}
...
function getLayoutConfig(card, attrs) {
    ...
    let ctx = getCtx();
    let deck = attrs.deck || (ctx ? ctx.viewingDeck : null);
    // Also check game state for in-game rendering
    if (!deck && ctx && ctx.gameState) {
        deck = ctx.gameState.deck;
    }
    if (!deck) return null;
...
function CardFace() {
    return {
        view(vnode) {
            ...
            let D = getDesigner();
            if (layoutConfig && D && D.LayoutRenderer) {
