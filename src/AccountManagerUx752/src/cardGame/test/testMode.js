/**
 * CardGame Test Mode - Comprehensive Test Suite (ESM)
 ...
 *
 * ESM port of Ux7 testMode.js. All window.CardGame.X references replaced
 * with _NS.X (namespace object passed via init()).
 *
 * Depends on (via NS):
 *   - NS.Constants (STATUS_EFFECTS, COMBAT_OUTCOMES)
 *   - NS.Storage (deckStorage, gameStorage, campaignStorage, encodeJson, decodeJson,
 *       createCampaignData)
 *   - NS.Engine (rollD20, getActorATK, getActorDEF, getStackSkillMod, checkGameOver,
 ...
 *   - NS.AI (CardGameLLM, CardGameNarrator, checkLlmConnectivity, llmStatus,
 *       initializeLLMComponents, CardGameVoice)
 *   - NS.Rendering (CardFace)
 *   - NS.Themes (loadThemeConfig, activeTheme)
 *   - NS.ctx (shared mutable context: screen, viewingDeck, gameState,
 *       gameNarrator, gameDirector, gameChatManager, gameVoice, gameAnnouncerVoice)
 *
 * Exports: testMode, TEST_CARDS, testState, runTestSuite, TestModeUI, init
 */

import m from 'mithril';

// ── NS reference (set via init()) ───────────────────────────────────────
let _NS = null;
...
function C()       { return _NS?.Constants; }
function S()       { return _NS?.Storage; }
function Eng()     { return _NS?.Engine || {}; }
function AI()      { return _NS?.AI || {}; }
function R()       { return _NS?.Rendering || {}; }
function Themes()  { return _NS?.Themes || {}; }
function GS()      { return _NS?.GameState || {}; }
function ctx()     { return _NS?.ctx || {}; }
...
async function runTestBody() {
    ...
    if (!cats || cats.length === 0) cats = Object.keys(TEST_CATEGORIES);
    ...
    if (cats.includes("modules")) {
        ...
        let namespaces = [
            ["NS.Constants",  _NS.Constants],
            ["NS.Storage",    _NS.Storage],
            ["NS.Engine",     _NS.Engine],
            ["NS.GameState",  _NS.GameState],
            ["NS.Characters", _NS.Characters],
            ["NS.Themes",     _NS.Themes],
            ["NS.AI",         _NS.AI],
            ["NS.ArtPipeline", _NS.ArtPipeline],
            ["NS.Rendering",  _NS.Rendering],
            ["NS.UI",         _NS.UI],
            ["NS.Designer",   _NS.Designer],
        ...
        if (ctxObj) {
            ...
            for (let prop of artProps) {
                try {
                    let v = ctxObj[prop];
                    let artState = _NS.ArtPipeline?.state;
    ...
    if (cats.includes("gameflow")) {
        ...
        if (gameState.createGameState) {
            ...
            if (!gs) {
                ...
                if (_NS.ctx) _NS.ctx.gameState = gs;
                ...
                // Restore game state
                if (_NS.ctx) _NS.ctx.gameState = savedGs;
            }
        } else {
    ...
    if (cats.includes("storage")) {
        ...
        try {
            ...
            let LC = _NS.Designer ? _NS.Designer.LayoutConfig : null;
            if (resolvedDeck && LC) {
    ...
    if (cats.includes("narration")) {
        ...
        try {
            ...
            else if (_NS.ctx) _NS.ctx.gameState = tempGs;
            ...
            else if (_NS.ctx) _NS.ctx.gameState = savedGs;
        } catch (e) {
        ...
        try {
            ...
            else if (_NS.ctx) _NS.ctx.gameState = tempGs2;
            ...
            else if (_NS.ctx) _NS.ctx.gameState = savedGs2;
    ...
    if (cats.includes("combat")) {
        ...
        if (_NS.ctx) _NS.ctx.gameState = tempGs;
    ...
    if (cats.includes("voice")) {
        ...
        if (!CardGameVoice) {
            ...
            // In ESM mode, check for audio via _NS.ctx or a global page reference
            let page = _NS.ctx?.page;
            ...
            else if (_NS.ctx) _NS.ctx.gameState = tempVoiceGs;
            ...
            else if (_NS.ctx) _NS.ctx.gameState = savedVoiceGs;
    ...
    if (cats.includes("llm") || cats.includes("voice")) {
        ...
        if (resolvedDeck && initializeLLMComponents) {
            ...
            if (testGs) {
                if (_NS.ctx) _NS.ctx.gameState = testGs;
    ...
    if (cats.includes("ux")) {
        ...
        if (engine.decrementWeaponDurability && engine.decrementApparelDurability) {
            ...
            // G2: Weapon breaks at durability 1
            {
                ...
                let actor = makeTestActor();
                actor.equipped.handR = w;
                let savedCtxGs = _NS.ctx ? _NS.ctx.gameState : null;
                ...
                if (_NS.ctx) _NS.ctx.gameState = savedCtxGs;
            }
        ...
        if (gameState.createGameState && gameState.advancePhase) {
            ...
            if (pgs) {
                if (_NS.ctx) _NS.ctx.gameState = pgs;
    ...
    if (cats.includes("layouts")) {
        ...
        if (LC && CARD_SIZES && CARD_TYPES) {
            // L2: Default layout generation for each card type x size
    ...
    if (cats.includes("designer")) {
        ...
        if (resolvedDeck) {
            try {
                let context2 = ctx();
                let origScreen = context2.screen;
                let origDeck = context2.viewingDeck;
                context2.viewingDeck = resolvedDeck;
                context2.screen = "deckView";
                let container = document.createElement("div");
                container.style.cssText = "position:absolute;top:-9999px;left:-9999px;width:800px;height:600px;";
                document.body.appendChild(container);
                let DeckView = _NS.UI.DeckView;
                if (DeckView) {
                    m.render(container, m(DeckView));
                    deckViewHTML = container.innerHTML;
                }
                document.body.removeChild(container);
                context2.screen = origScreen;
                context2.viewingDeck = origDeck;
            } catch (e) { /* skip */ }
        }
    ...
    if (cats.includes("export")) {
        ...
        if (CARD_SIZES) {
            ...
            for (let st of sizeTests) {
                ...
                if (size) {
                    ...
                    testLog("export", "E3: " + st.key + " size " + size.px[0] + "x" + size.px[1] + " expected " + st.expected[0] + "x" + st.expected[1],
                        pxMatch ? "pass" : "fail");
                }
            }
        }
        ...
        if (LR && LC && hasHtml2Canvas) {
            try {
                ...
                testLog("export", "E4a: Offscreen render at " + pxW + "x" + pxH + " produced content=" + rendered,
                    rendered ? "pass" : "fail");
                ...
                try {
                    ...
                    testLog("export", "E4b: html2canvas capture " + canvas.width + "x" + canvas.height,
                        canvasOk ? "pass" : "warn");
                } catch (e) {
        ...
        if (LR && LC && CARD_SIZES) {
            ...
            testLog("export", "E7: All types x all sizes: " + renderCount + "/" + expected,
...
// ── Test Mode UI Component (self-contained for ESM) ──────────────────
function TestModeUI() {
    return {
        view: function() {
            let context = ctx();
            ...
            function statusColor(status) {
                ...
                if (status === "skip") return "#9e9e9e";
                return "#2196f3";
            }

            // Filter logs
            let filteredLogs = testState.logs;
            if (testState.logFilter !== "all") {
                filteredLogs = testState.logs.filter(function(l) {
                    return l.status === testState.logFilter;
                });
            }

            return m("div", { class: "cg2-test-mode", style: "padding: 16px; font-family: monospace;" }, [
                // ── Toolbar ──
                m("div", { style: "display:flex; align-items:center; gap:12px; margin-bottom:16px;" }, [
                    m("button", {
                        onclick: function() {
                            if (_NS.ctx) _NS.ctx.screen = testDeck ? "deckView" : "deckList";
                            m.redraw();
                        },
                        style: "padding:6px 12px; cursor:pointer;"
                    }, [
                        m("span", { class: "material-icons", style: "font-size:18px; vertical-align:middle;" }, "arrow_back"),
                        " Back"
                    ]),
                    m("h2", { style: "margin:0; flex:1;" }, "Card Game Tests"),
                    testDeckName ? m("span", { style: "color:#888;" }, "Deck: " + testDeckName) : null
                ]),

                // ── Category Toggles ──
                m("div", { style: "display:flex; flex-wrap:wrap; gap:8px; margin-bottom:16px;" }, [
                    m("button", {
                        onclick: function() {
                            testState.selectedCategories = Object.keys(TEST_CATEGORIES);
                            m.redraw();
                        },
                        style: "padding:4px 8px; cursor:pointer; font-size:12px;"
                    }, "All"),
                    m("button", {
                        onclick: function() {
                    ...
                    Object.keys(TEST_CATEGORIES).map(function(key) {
                        let cat = TEST_CATEGORIES[key];
                        let selected = (testState.selectedCategories || []).includes(key);
                        return m("label", { style: "display:flex; align-items:center; gap:4px; cursor:pointer; padding:4px 8px; border-radius:4px; background:" + (selected ? "#e3f2fd" : "#f5f5f5") + ";" }, [
                            m("input", {
                                type: "checkbox",
                                checked: selected,
                                onchange: function() {
                                    if (!testState.selectedCategories) testState.selectedCategories = [];
                                    if (selected) {
                                        testState.selectedCategories = testState.selectedCategories.filter(function(c) { return c !== key; });
                                    } else {
                                        testState.selectedCategories.push(key);
                                    }
                                    m.redraw();
                                }
                            }),
                            m("span", { class: "material-icons", style: "font-size:16px;" }, cat.icon),
                            cat.label
                        ]);
                    })
                ]),
                    ...
                    m("button", {
                        onclick: function() {
                            if (!testState.running) {
                                if (!testState.selectedCategories || testState.selectedCategories.length === 0) {
                                    testState.selectedCategories = Object.keys(TEST_CATEGORIES);
                                }
                                runTestSuite();
                            }
                        ...
                        style: "padding:8px 24px; font-size:14px; cursor:" + (testState.running ? "not-allowed" : "pointer") + "; background:#1976d2; color:white; border:none; border-radius:4px;"
                    }, testState.running ? "Running..." : "Run Tests"),
                    testState.currentTest ? m("span", { style: "margin-left:12px; color:#666;" }, testState.currentTest) : null
                ]),

                // ── Results Summary ──
                m("div", { style: "display:flex; gap:16px; margin-bottom:16px; padding:12px; background:#f5f5f5; border-radius:8px;" }, [
                    m("span", { style: "color:#4caf50; font-weight:bold;" }, "Pass: " + testState.results.pass),
                    m("span", { style: "color:#f44336; font-weight:bold;" }, "Fail: " + testState.results.fail),
                    m("span", { style: "color:#ff9800; font-weight:bold;" }, "Warn: " + testState.results.warn),
                    m("span", { style: "color:#9e9e9e;" }, "Skip: " + (testState.results.skip || 0)),
                    testState.completed ? m("span", { style: "margin-left:auto; font-weight:bold; color:" + (testState.results.fail > 0 ? "#f44336" : "#4caf50") + ";" },
                        testState.results.fail > 0 ? "FAILED" : "PASSED") : null
                ]),

                // ── Log Filter ──
                m("div", { style: "margin-bottom:8px; display:flex; gap:8px;" }, [
                    ["all", "pass", "fail", "warn", "info"].map(function(f) {
                        return m("button", {
                            onclick: function() { testState.logFilter = f; m.redraw(); },
                            style: "padding:4px 8px; cursor:pointer; font-size:12px; border-radius:4px; background:" + (testState.logFilter === f ? "#e3f2fd" : "#f5f5f5") + "; border:1px solid " + (testState.logFilter === f ? "#1976d2" : "#ddd") + ";"
                ...
                // ── Log Console ──
                m("div", {
                    style: "max-height:400px; overflow-y:auto; background:#1e1e1e; color:#d4d4d4; padding:12px; border-radius:8px; font-size:12px; line-height:1.6;"
                }, [
                    filteredLogs.length === 0
                        ? m("div", { style: "color:#666;" }, "No log entries yet. Click 'Run Tests' to start.")
                        : filteredLogs.map(function(entry) {
                            return m("div", { style: "border-bottom:1px solid #333; padding:2px 0;" }, [
                                m("span", { style: "color:#666;" }, entry.time + " "),
                                entry.category ? m("span", { style: "color:#9cdcfe;" }, "[" + entry.category + "] ") : null,
                                m("span", { style: "color:" + statusColor(entry.status) + "; font-weight:" + (entry.status === "fail" ? "bold" : "normal") + ";" },
                                    "[" + entry.status.toUpperCase() + "] "),
                                m("span", entry.message)
                            ]);
                        })
                ])
            ]);
...
export { testMode, TEST_CARDS, testState, runTestSuite, TestModeUI, init };
export default testMode;

console.log("[CardGame] test/testMode loaded (ESM)");
