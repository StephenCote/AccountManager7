/**
 * CardGame Designer — Layout Renderer (ESM)
 ...
 *
 * Depends on:
 *   - gameConstants (CARD_TYPES, CARD_SIZES, LAYOUT_ZONES)
 *   - layoutConfig (resolveTemplate, deriveGameStats, getLayout)
 *   - rendering module (showImagePreview — for image click)
 */
import m from 'mithril';
import { gameConstants as C } from '../constants/gameConstants.js';
import { layoutConfig as LC } from './layoutConfig.js';

// Late-bound access to rendering module (may not be loaded yet)
let _rendering = null;
function R() { return _rendering || {}; }

/** Set the rendering module reference (call from CardGameApp wiring) */
export function setRendering(renderingModule) {
    _rendering = renderingModule;
}
...
console.log("[CardGame] Designer.LayoutRenderer loaded (ESM)");
