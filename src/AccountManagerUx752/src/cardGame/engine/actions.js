 ...
 *
 * Depends on: gameConstants (GAME_PHASES, ACTION_DEFINITIONS, COMMON_ACTIONS)
 */
...
export function anteCard(state, actor, actorName) {
...
export function claimPot(state, winner) {
...
export function addToPot(state, card, reason) {
    if (!card) return;
    state.pot.push(card);
...
export function addToRoundLoot(state, item, source) {
    if (!item) return;
...
export function drawCardsForActor(actor, count) {
 ...
 */
export function getActionsForActor(actor) {
 ...
 */
export function isActionPlacedThisRound(state, actionName, owner) {
 ...
 */
export function selectAction(state, positionIndex, actionName) {
 ...
 */
export function canModifyAction(coreCard, modifierCard) {
...
export function placeCard(state, positionIndex, card, forceModifier) {
    ...
    if (!forceModifier && hasExistingCore) {
        ...
        if (card.type === "action" || card.type === "talk" || card.type === "magic") {
            ...
            let pg = getPage();
            if (pg && pg.toast) pg.toast("warn", "Position already has an action card");
            return false;
    ...
    if (isModifier) {
        ...
        if (!compat.allowed) {
            ...
            let pg = getPage();
            if (pg && pg.toast) pg.toast("warn", compat.reason);
            return false;
        }
        ...
        if (existingSameType) {
            ...
            let pg = getPage();
            if (pg && pg.toast) pg.toast("warn", "Already has a " + typeLabel + " card in this action");
            return false;
        }
        ...
        if (modType === coreType) {
            ...
            let pg = getPage();
            if (pg && pg.toast) pg.toast("warn", "Cannot stack two " + typeLabel + " cards");
            return false;
        }
        ...
        if (card._fromPicker && isActionPlacedThisRound(state, card.name, currentPlayer)) {
            ...
            let pg = getPage();
            if (pg && pg.toast) pg.toast("warn", card.name + " already placed this round");
            return false;
        }
...
export function removeCardFromPosition(state, positionIndex, skipRedraw) {
    if (!state) return false;
    if (state.phase !== C.GAME_PHASES.DRAW_PLACEMENT) return false;
...
// ── Deal Initial Stack ──────────────────────────────────────────────
export function dealInitialStack(apparelCards, itemCards) {
    let stack = [];
