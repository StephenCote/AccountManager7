 ...
 *
 * Depends on: gameConstants (STATUS_EFFECTS)
 */
...
export function applyStatusEffect(actor, effectId, sourceName) {
...
export function removeStatusEffect(actor, effectId) {
...
export function hasStatusEffect(actor, effectId) {
...
export function getStatusModifiers(actor) {
...
export function processStatusEffectsTurnStart(actor) {
    if (!actor.statusEffects) return [];
    let messages = [];
    actor.statusEffects.forEach(effect => {
        let def = C.STATUS_EFFECTS[effect.id.toUpperCase()];
...
export function onActorHit(actor) {
    if (!actor.statusEffects) return;
    actor.statusEffects = actor.statusEffects.filter(effect => {
...
// Note: uses late-bound references to sibling engine functions via the engine object
export function applyParsedEffects(parsed, owner, target, sourceName, engine) {
    let log = [];
    if (parsed.damage && target) {
        let dmgResult = engine.applyDamage(target, parsed.damage);
    ...
    if (parsed.draw && owner) {
        engine.drawCardsForActor(owner, parsed.draw);
...
export function getStackSkillMod(stack, actionType) {
