/**
 * markdownConverter.js — Markdown/HTML bidirectional converter (ESM)
 * Port of Ux7 client/markdownConverter.js
 *
 * Uses TurndownService (HTML→Markdown) and marked (Markdown→HTML).
 * Also handles legacy BB code migration.
 */
...
const bbCodePattern = /\[(b|i|s|p|div|url|img|ul|ol|li|h[1-5]|blockquote)\b[^\]]*\]/i;
...
const markdownConverter = {
    /** Markdown (or legacy BB code) → HTML string */
    toHtml: function(s) {
        if (!s || !s.trim()) return '';
        return marked.parse(normalize(s));
    },

    /** DOM childNodes array → Markdown string */
