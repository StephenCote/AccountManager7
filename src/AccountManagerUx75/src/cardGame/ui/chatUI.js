/**
 * CardGame UI - Talk/Chat Interface (ESM)
...
function TalkChatUI() {
    ...
    async function sendMessage() {
        ...
        let gameChatManager = ctxObj.gameChatManager;
        let gameVoice = ctxObj.gameVoice;
        ...
        if (gameState) {
            const buildEmotionContext = NS?.AI?.buildEmotionContext;
            gameContext = {
    ...
    function endChat() {
        ...
        const playerMsgs = gameState.chat.messages.filter(msg => msg.role === "player").length;
        ...
        if (msgCount >= 4 && playerMsgs >= 2) {
            ...
            getPage().toast("info", "Productive conversation! +2 morale");
        } else if (msgCount >= 2) {
            gameState.player.morale = Math.min(gameState.player.maxMorale, gameState.player.morale + 1);
            getPage().toast("info", "Brief exchange. +1 morale");
        }
        ...
        if (gameState.phase === GAME_PHASES.RESOLUTION) {
            setTimeout(() => NS.GameState.advanceResolution(), 500);
        }
    }
