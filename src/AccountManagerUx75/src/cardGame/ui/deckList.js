...
function ctx() { return NS?.ctx || {}; }
function storage() { return NS?.Storage || {}; }
function themes() { return NS?.Themes || {}; }
function chars() { return NS?.Characters || {}; }
...
async function loadSavedDecks() {
    ...
    m.redraw();
    try {
        getClient().clearCache("auth.group");
        ...
        for (let name of names) {
            ...
            if (data) {
                ...
                decks.push({
                    ...
                    lastSaveRound: savesList.length > 0 ? null : null,
...
async function saveDeck(deck) {
    ...
    if (selectedChars && selectedChars.length > 0) {
        ...
        if (unpersisted.length > 0) {
            getPage().toast("info", "Saving characters...");
    ...
    if (prevCharGroupName && prevCharGroupName !== safeName) {
        try {
            let oldGroup = await getPage().findObject("auth.group", "data", "~/CardGame/" + prevCharGroupName);
            if (oldGroup) {
                oldGroup.name = safeName;
                await getPage().patchObject(oldGroup);
    ...
    if (result) {
        getPage().toast("success", "Deck saved: " + deck.deckName);
        ctx().builtDeck = null;
        chars().state.selectedChars = [];
        m.redraw();
        await viewDeck(safeName);
    } else {
        getPage().toast("error", "Failed to save deck");
        m.redraw();
    }
}
...
async function deleteGameChats(storageName) {
    if (!storageName) return 0;
    let am7chat = NS?.am7chat;
    let am7view = NS?.am7view;
    if (!am7chat) return 0;
    try {
        let dir = await getPage().findObject("auth.group", "DATA", "~/ChatRequests");
        if (!dir) return 0;
        ...
        let cgChatDir = await getPage().findObject("auth.group", "DATA", "~/CardGame/Chats");
        if (cgChatDir) {
            let cq = am7view.viewQuery(am7model.newInstance("olio.llm.chatConfig"));
            cq.field("groupId", cgChatDir.id);
            cq.cache(false);
            cq.range(0, 100);
            let cqr = await getPage().search(cq);
            if (cqr && cqr.results) {
                ...
                for (let cfg of cfgMatches) {
                    try {
                        await getPage().deleteObject(cfg[am7model.jsonModelKey], cfg.objectId);
                        deleted++;
                    } catch (e) {
...
async function deleteDeck(storageName) {
    ...
    if (chatCount > 0) {
        getPage().toast("info", "Deleted " + chatCount + " game chat(s)");
    }
    let deckStorage = storage().deckStorage;
    let ok = await deckStorage.remove(storageName);
    if (ok) {
        getPage().toast("success", "Deck deleted");
        await loadSavedDecks();
    } else {
        getPage().toast("error", "Failed to delete deck");
    }
    m.redraw();
}
...
async function viewDeck(storageName) {
    ...
    if (data) {
        ...
        let getThreatCreatures = NS?.Engine?.getThreatCreatures;
        ...
        let getScenarioCards = NS?.Engine?.getScenarioCards;
        ...
        getPage().toast("error", "Failed to load deck");
    }
    m.redraw();
}
