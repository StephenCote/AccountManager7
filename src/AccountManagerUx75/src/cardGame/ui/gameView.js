/**
 * CardGame UI - Game View Components (ESM)
...
function rendering() { return NS?.Rendering || {}; }
function GS() { return NS?.GameState || {}; }
function E() { return NS?.Engine || {}; }
function AI() { return NS?.AI || {}; }
...
function GameView() {
    return {
        ...
        view() {
            let GAME_PHASES = NS?.Constants?.GAME_PHASES || {};
            let ACTION_DEFINITIONS = NS?.Constants?.ACTION_DEFINITIONS || {};
            let gameState = ctx().gameState;
            let gameCharSelection = ctx().gameCharSelection;
            let viewingDeck = ctx().viewingDeck;
            ...
            let InitiativePhaseUI = NS?.UI?.InitiativePhaseUI;
            // v3: EquipPhaseUI removed — equipment changes are "Use Item" actions
            let ThreatResponseUI = NS?.UI?.ThreatResponseUI;
            let CleanupPhaseUI = NS?.UI?.CleanupPhaseUI;
            let TalkChatUI = NS?.UI?.TalkChatUI;
            let GameOverUI = NS?.UI?.GameOverUI;
                            ...
                            (gameState.narrationText && gameState.phase === GAME_PHASES.RESOLUTION)
                                ? m("div", { class: "cg2-resolution-narration" }, [
                                    ...
                                    m("button", {
                                        ...
                                        title: "Skip",
                                        onclick(e) { e.stopPropagation(); NS.GameState.skipNarration(); }
                ...
                m("div", { class: "cg2-fanned-cards" },
                    cardList.slice(0, maxShow).map((card, i) => {
                        ...
                        return m("div", {
                            ...
                            onclick(e) {
                                e.stopPropagation();
                                let showCardPreview = NS?.Rendering?.showCardPreview;
...
function CharacterSidebar() {
    return {
        view(vnode) {
            let STATUS_EFFECTS = NS?.Constants?.STATUS_EFFECTS || {};
                            ...
                            ondrop(e) {
                                ...
                                try {
                                    ...
                                    if (pos.stack && pos.stack.coreCard) {
                                        ...
                                        if (!isModifier) {
                                            getPage().toast("warn", "Use the icon picker for actions");
                                            return;
                                        }
                                        let compat = canModifyAction(pos.stack.coreCard, card);
                                        if (!compat.allowed) {
                                            getPage().toast("warn", compat.reason);
                                            return;
                                        }
                                        ...
                                        if (!canPlace) {
                                            getPage().toast("warn", "Choose an action first for this card type");
                                            return;
                                        }
                            ...
                            onclick(e) {
                                ...
                                if (pos.stack && pos.stack.coreCard) {
                                    ...
                                    if (canPlace) {
                                        ...
                                        getPage().toast("info", "Choose an action first, then tap to add this card as a modifier");
                                    }
                                }
                                m.redraw();
                            }
