 ...
 *
 * Depends on: gameConstants (COMBAT_OUTCOMES), effects module
 */
import { gameConstants as C } from '../constants/gameConstants.js';
import * as Effects from './effects.js';
import { addToRoundLoot } from './actions.js';
...
export function rollInitiative(stats) {
...
export function getActorATK(actor) {
    let total = 0;
    let counted = new Set();
...
export function getWeaponATKs(actor) {
    let weapons = [];
    let seen = new Set();
...
export function getActorDEF(actor) {
    let total = 0;
    let counted = new Set();
    // From equipped slots (primary source)
    if (actor.equipped) {
...
export function rollAttack(attacker, stack) {
    let raw = rollD20();
    let stats = attacker.character.stats || {};
    let strMod = stats.STR || 0;
    let atkBonus = getActorATK(attacker);
    let skillMod = Effects.getStackSkillMod(stack, "attack");

    // Add status effect modifiers
    let statusMods = Effects.getStatusModifiers(attacker);
...
export function rollDefense(defender) {
    let raw = rollD20();
    ...
    // Add status effect modifiers
    let statusMods = Effects.getStatusModifiers(defender);
...
export function getCombatOutcome(attackRoll, defenseRoll) {
...
export function calculateDamage(attacker, outcome) {
    let stats = attacker.character.stats || {};
    let strMod = stats.STR || 0;
...
export function resolveCombat(attackerActor, defenderActor, stack) {
    ...
    if (outcome.damageMultiplier > 0) {
        ...
        if (outcome.isCriticalHit) {
            ...
            // Import encounters.generateCriticalReward lazily to avoid circular dep
            let { generateCriticalReward } = await_generateCriticalReward();
            ...
            if (defenderActor.cardStack && defenderActor.cardStack.length > 0 && Math.random() < 0.5) {
                ...
                if (droppableItems.length > 0) {
                    ...
                    if (idx >= 0) {
                        ...
                        let gs = _gs.getGameState();
                        if (gs) {
                            addToRoundLoot(gs, droppedItem, "critical hit drop");
                        }
        ...
        if (outcome.isCriticalCounter) {
            Effects.applyStatusEffect(attackerActor, "stunned", "Critical Counter");
...
// Late-bound import for encounters module to avoid circular dependency
// encounters imports from effects, combat imports from effects, encounters imports from combat
let _encountersModule = null;
function await_generateCriticalReward() {
    if (!_encountersModule) {
        // Synchronous access -- encounters module must already be loaded
        // This is resolved at module evaluation time in the bundler
        _encountersModule = require_encounters();
    }
    return _encountersModule;
}

let _encountersResolver = null;
export function _setEncountersModule(mod) {
    _encountersModule = mod;
}
function require_encounters() {
    // Fallback: return stub that generates basic rewards
    return {
        generateCriticalReward(type, actor) {
...
export function decrementApparelDurability(actor, isCritical) {
    let broken = [];
...
export function decrementWeaponDurability(actor) {
    let broken = [];
    if (!actor.equipped) return broken;
