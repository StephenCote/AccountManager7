 ...
 *
 * Exports: CardGameDirector, applyAIDecision, aiPlaceCards,
 *          checkPlacementComplete, getDirector, setDirector
 */
...
import {
    ...
    selectAction, isCoreCardType, placeCard, canModifyAction
} from '../engine/actions.js';
import { checkGameOver } from '../engine/combat.js';
...
function advancePhase()      { return _gs.advancePhase?.(); }
...
Reply with ONLY the JSON object, no markdown or text.`;
    ...
    async requestPlacement(gameState) {
        ...
        try {
            ...
            getPage().toast("warn", "AI decision unclear, using default");
            return this._fifoFallback(gameState);
            ...
            if (this.consecutiveErrors === 1) {
                getPage().toast("warn", "AI unavailable, using default placement");
            }
...
async function aiPlaceCards() {
    ...
    for (let mod of remainingMods) {
        ...
        for (let posIdx of positions) {
            ...
            if (canModifyAction) {
                let compat = canModifyAction(pos.stack.coreCard, mod);
                if (!compat.allowed) continue;
            }
            if (placeCard(gameState, posIdx, mod, true)) {
