/**
 * CardGame Designer — Designer Canvas (ESM)
 ...
 *
 * Depends on:
 *   - gameConstants (CARD_TYPES, CARD_SIZES, LAYOUT_ZONES, LAYOUT_ELEMENT_TYPES)
 *   - layoutConfig (getLayout, saveLayout, cloneLayout, addElement,
 *       removeElement, moveElement, findElement, updateElementStyle, updateZoneHeight)
 *   - layoutRenderer (LayoutCardFace)
 *   - iconPicker (IconPickerButton, IconPickerOverlay)
 *   - storage (deckStorage)
 */
import m from 'mithril';
import { am7model } from '../../core/model.js';
import { gameConstants as C } from '../constants/gameConstants.js';
import { layoutConfig as LC } from './layoutConfig.js';
import { layoutRenderer as LR } from './layoutRenderer.js';
import { iconPicker as IP } from './iconPicker.js';

// Late-bound page access
function getPage() { return am7model._page; }

// Late-bound storage access (set via setStorage or from CardGameApp wiring)
let _storage = null;
export function setStorage(storageModule) {
        ...
        view: function(vnode) {
            ...
            return m("div", { class: "cg2-dz-props" }, [
                ...
                el.type === "emoji" ? m("div", { class: "cg2-dz-prop-row" }, [
                    ...
                    (function() {
                        let pg = getPage();
                        return pg && pg.components && pg.components.emoji ?
                            m(pg.components.emoji.component) :
                            m("input", {
                                type: "text", value: el.content || "",
...
function DesignerView() {
    // Local reference for ctx — will be set via attrs or late-bound
    let _ctx = null;
    function ctx() { return _ctx || {}; }

    return {
        oninit: function(vnode) {
            _ctx = vnode.attrs.ctx || null;
            let deck = vnode.attrs.deck || ctx().designerDeck || ctx().viewingDeck;
            if (deck) loadWorkingLayout(deck);
        },
        view: function(vnode) {
            _ctx = vnode.attrs.ctx || _ctx;
            let ctxObj = ctx();
            let deck = ctxObj.designerDeck || ctxObj.viewingDeck;
            if (!deck) return m("div", { style: { padding: "20px", color: "#aaa" } }, [
                m("button", {
                    class: "cg2-btn",
                    onclick: function() { ctxObj.screen = "deckList"; m.redraw(); }
                }, "\u2190 Back to Decks"),
            ...
            return m("div", { class: "cg2-designer" }, [
                ...
                m("div", { class: "cg2-toolbar" }, [
                    m("button", {
                        class: "cg2-btn",
                        onclick: function() { ctxObj.screen = "deckView"; m.redraw(); }
                    }, "\u2190 Back to Deck"),
                    ...
                    m("button", {
                        ...
                        async onclick() {
                            ...
                            let stor = getStorage();
                            if (stor && stor.deckStorage) {
                                await stor.deckStorage.save(safeName, deck);
                            }
                            designerState.dirty = false;
                            let pg = getPage();
                            if (pg && pg.toast) pg.toast("success", "Layout saved");
                            m.redraw();
                        }
                    }, [
...
// Also export DesignerView directly for CardGameApp routing convenience
export { DesignerView };

console.log("[CardGame] Designer.DesignerCanvas loaded (ESM)");
