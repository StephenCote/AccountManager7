/**
 * CardGame Designer — Export Dialog (ESM)
 * Modal dialog for configuring and executing card export.
 *
 * Ported from Ux7 IIFE to ESM.
 *
 * Depends on:
 *   - gameConstants (CARD_SIZES)
 *   - exportPipeline (exportDeck, cancelExport, exportState)
 */
import m from 'mithril';
import { am7model } from '../../core/model.js';
import { gameConstants as C } from '../constants/gameConstants.js';
import { exportPipeline as EP } from './exportPipeline.js';
                            ...
                            async onclick() {
                                try {
                                    ...
                                    if (result) {
                                        let pg = getPage();
                                        if (pg && pg.toast) {
                                            pg.toast("success", "Exported " + result.count + " cards (" + Math.round(result.size / 1024) + " KB)");
                                        }
                                        closeDialog();
                                    }
                                } catch (e) {
                                    let pg = getPage();
                                    if (pg && pg.toast) {
                                        pg.toast("error", "Export failed: " + e.message);
                                    }
                                }
                            }
                        }, [
...
console.log("[CardGame] Designer.ExportDialog loaded (ESM)");
