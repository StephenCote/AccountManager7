...
public class LoginService {
	...
	private BaseRecord getAuthenticatedToken(BaseRecord authnRequest, HttpServletRequest request){
		...
		if(authnRequest != null) {
			...
			String orgPath = authnRequest.get(FieldNames.FIELD_ORGANIZATION_PATH);
			if(orgPath == null) {
				orgPath = "/Public";
			}
			...
			if(org == null) {
				...
				String subject = authnRequest.get(FieldNames.FIELD_SUBJECT);
				if(subject == null) {
					logger.error("Invalid subject");
				}
				String subType = authnRequest.get(FieldNames.FIELD_SUBJECT_TYPE);
				if(subType == null) {
					subType = ModelNames.MODEL_USER;
				}
				BaseRecord sub = null;
				if(subType.equals(ModelNames.MODEL_USER)) {
					sub = IOSystem.getActiveContext().getRecordUtil().getRecord(null, subType, subject, 0L, 0L, org.get(FieldNames.FIELD_ID));
				}
				else {
					sub = IOSystem.getActiveContext().getRecordUtil().getRecordByUrn(null, subType, subject);
				}
				if(sub == null) {
