...
public class AM7LoginModule implements LoginModule {
	...
	public boolean login() throws LoginException {
		...
		if(username.equals("${jwt}")) {
			username = TokenService.validateTokenToSubject(password);
			tokenAuth = true;
		}
		...
		if (orgType != null) {
			user = IOSystem.getActiveContext().getRecordUtil().getRecord(null, ModelNames.MODEL_USER, username, 0L, 0L,
					orgType.get(FieldNames.FIELD_ID));
			if (user != null && !tokenAuth) {
				VerificationEnumType vet = VerificationEnumType.UNKNOWN;
				BaseRecord cred = getLatestCredential(user);
				if (cred != null) {
					try {
						vet = IOSystem.getActiveContext().getFactory().verify(user, getLatestCredential(user),
								ParameterUtil.newParameterList("password", password));
	...
	public static List<RolePrincipal> getRoleSets(UserPrincipal uprince) throws LoginException, FactoryException {
		...
		try {
			List<BaseRecord> entsp = IOSystem.getActiveContext().getMemberUtil().getParticipations(uprince,
					ModelNames.MODEL_ROLE);
