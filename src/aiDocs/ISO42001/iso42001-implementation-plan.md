...
**Keystore topology finding (corrected — supersedes the earlier "non-fatal noise" hand-wave):** the tests share Objects7's `am7db`. Objects7's `am7/.jks` holds only the **default** orgs (1–13); the `/ISO42001` org is a **new** org (14) whose keystore was generated under the **ISO-module-local** `am7/.jks/14`. So `DEFAULT_FILE_BASE` must stay `./am7` (module-local). Pointing it at `../AccountManagerObjects7/am7` like Agent7 does (Agent7 reuses the default `/Development` org) was tried and **fails every ISO test** — it finds 1–13 but not the ISO org's keystore. The residual `Key store does not exist at ./am7/.jks/{1,2,3}/...` + "Organization already exists" log lines are confined to the unused default orgs (verified: only orgs 1/2/3 appear) and are non-fatal; the `/ISO42001` org initializes cleanly. The clean elimination is a **dedicated ISO test database (option #2)** — noted as a TODO in `ISO42001BaseTest`'s javadoc; deferred until the db is provisioned.
...
- `DEFAULT_FILE_BASE` kept at `./am7` (NOT Objects7's am7) — see keystore finding above; option #1 (Agent7's base path) was tested and rejected because the ISO org's keystore is module-local.

**DoD:** `cd AccountManagerISO42001 && mvn -o compile` → BUILD SUCCESS; `mvn -o test -Dtest=TestISO42001AnalysisProfile,TestISO42001Models` → **BUILD SUCCESS — Tests run: 9, Failures: 0, Errors: 0** (4 analysisProfile + 5 Phase-1 models, the latter re-run green on the refactored base). First live-DB exercise of the analysisProfile model + the new `testConfig.analysisProfile` column.
...
| 2026-06-22 | 3a | Extracted `ISO42001BaseTest` (shared scaffolding; both ISO integration tests now extend it). Investigated the keystore log noise: Objects7's `am7/.jks` holds default orgs 1–13; `/ISO42001` is org 14 with a module-local keystore, so `DEFAULT_FILE_BASE` must stay `./am7` — tested Agent7's `../AccountManagerObjects7/am7` path and it **fails all ISO tests** (ISO org keystore not there); reverted. Residual log misses confined to unused orgs 1/2/3 (non-fatal). Added dedicated-ISO-test-DB (option #2) TODO. `mvn -o test -Dtest=TestISO42001AnalysisProfile,TestISO42001Models` | **BUILD SUCCESS — Tests run: 9, Failures: 0, Errors: 0** |
