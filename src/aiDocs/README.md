...
| `PictureBookWorkflowOverhaul.md` | **Workflow graph overhaul / correction (2026-09-19)** — the PB2 workflow canvas shows nothing because `picturebook.v2` is `false` in the deployed `web.xml`: zero artifacts and zero bindings have ever been written. Records the measured state, disambiguates the two things "PB1/PB2" means (book/world model vs workflow graph), recommends retiring PB1 as the *outcome* of a five-phase sequence rather than the first step, and carries Q1-Q11 for Stephen (**Q6 and Q8 ratified 2026-09-19**). **§6 is the long-form/novel workstream** — chapter-at-a-time ingest with a cast shared across the whole work, which needs the ownership inversion (one Olio world per *series*, not per *book*) that reverses `PictureBook2Plan.md` §3.5's copy decision | proposal, not started |
| `PictureBookAsyncJobDesign.md` | **Async background jobs** for the long PictureBook/ChapBook operations — shared `AsyncJobRegistry` + `JobService` polling, extraction checkpoint/resume across a restart, ChapBook progress & cancel. Closes KI-49 (the "504 with no server-side error" was nginx's `proxy_read_timeout`, not an app exception) | active |
