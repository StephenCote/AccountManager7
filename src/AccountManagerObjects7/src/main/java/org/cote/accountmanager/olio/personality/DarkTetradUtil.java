...
public class DarkTetradUtil {
	public static final Logger logger = LogManager.getLogger(DarkTetradUtil.class);
	/// Dark Cube
	...
	private static Map<String, String> tetradKeyTitles = new HashMap<>();
	static {
		tetradKeyTitles.put("MNPS", "maleficent");
		tetradKeyTitles.put("MNPs", "malevolent manipulator");
		tetradKeyTitles.put("MNpS", "cruel tactician");
		tetradKeyTitles.put("MNps", "manipulative narcissist");

		tetradKeyTitles.put("MnPS", "sadistic schemer");
		tetradKeyTitles.put("MnPs", "cold strategist");
		tetradKeyTitles.put("MnpS", "torturous manipulator");
		tetradKeyTitles.put("Mnps", "machiavellian");

		tetradKeyTitles.put("mNPS", "tyrannical sadist");
		tetradKeyTitles.put("mNPs", "psychopathic narcissist");
		tetradKeyTitles.put("mNpS", "vain tormentor");
		tetradKeyTitles.put("mNps", "narcissist");
	...
	///
	public static String getDarkTetradKey(BaseRecord per) {
		...
		double sad = per.get(OlioFieldNames.FIELD_SADISM);
		...
		if(sad > 0.5) buff.append("S");
		else buff.append("s");

		return buff.toString();
	}
	public static String getDarkTetradName(String key) {
		if(key != null && tetradKeyTitles.containsKey(key)) {
			return tetradKeyTitles.get(key);
		}
		return null;
	}
	...
	public static RollEnumType rollDeception(BaseRecord rec) {
		int decStat = DarkTetradUtil.getDeceptionStatistic(rec);
		return RollUtil.rollStat20(decStat);
	}
	public static RollEnumType rollCounterDeception(BaseRecord rec) {
		int cntStat = DarkTetradUtil.getDeceptionCounterStatistic(rec);
	...
	public static OutcomeEnumType ruleDarkTetrad(BaseRecord interaction, PersonalityProfile actor, PersonalityProfile interactor) {
