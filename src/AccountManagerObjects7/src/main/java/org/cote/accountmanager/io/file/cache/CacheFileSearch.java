...
public class CacheFileSearch extends FileSearch implements ICache {
	...
	public void clearCache(BaseRecord rec) {
		IndexEntry idx = null;
		try {
			idx = IOSystem.getActiveContext().getIndexManager().getInstance(rec.getSchema()).findIndexEntry(rec);
	...
	public void clearCacheByIdx(IndexEntry idx) {
		...
		cache.entrySet().removeIf(entry ->{
			...
			for(BaseRecord r : mr.getResults()) {
				if(type.equals(r.getSchema()) && RecordUtil.matchIdentityRecordsByIdx(idx, r)) {
