...
public class IOFactory {
	...
	public static IPath getPathUtil(IOContext context) {
		if(context.getIoType().equals(RecordIO.FILE)) {
			return new FilePathUtil(context);
		}
		else if(context.getIoType().equals(RecordIO.DATABASE)) {
			return new DBPathUtil(context);
		}
		logger.error("getPathUtil: Unhandled IO type: " + context.getIoType().toString());
		return null;
	}
