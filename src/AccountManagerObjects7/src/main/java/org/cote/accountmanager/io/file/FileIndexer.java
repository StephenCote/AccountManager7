...
public class FileIndexer {
	...
	private boolean matchIndex(BaseRecord query, IndexEntry idx) {
		...
		if(modelName != null && !itype.equals(modelName)) {
			...
			logger.error("No Query Model for " + idx.getSchema() + " != " + itype + "!");
			return false;
		}
	...
	private void applyRecordToEntry(BaseRecord rec, IndexEntry entry) throws FieldException, ModelNotFoundException, ValueException {
		...
		ModelSchema srec = RecordFactory.getSchema(rec.getSchema());
		...
		for(FieldSchema f : srec.getFields()) {
			...
			if(f.isIdentity() || f.isIndex()) {
				...
				if(rec.hasField(f.getName()) && rec.get(f.getName()) != null) {
					...
					logger.warn("Record " + rec.getSchema() + " does not define an indexed field: " + f.getName());
					// logger.warn(rec.toString());
				}
			}
		}
		entry.set(FieldNames.FIELD_TYPE, rec.getSchema());
	}
	...
	public IndexEntry updateIndexEntry(Index idx, BaseRecord rec) throws IndexException {
		...
		synchronized(indices) {
			if(!rec.hasField(FieldNames.FIELD_ID) && !rec.hasField(FieldNames.FIELD_OBJECT_ID)) {
				throw new IndexException(String.format(IndexException.NOT_INDEXABLE, rec.getSchema(), "name, id, or objectId"));
			}
	...
	public boolean removeIndexEntry(BaseRecord rec) throws IndexException {
		if(!rec.hasField(FieldNames.FIELD_ID) && !rec.hasField(FieldNames.FIELD_OBJECT_ID)) {
			throw new IndexException(String.format(IndexException.NOT_INDEXABLE, rec.getSchema()));
		}
	...
	public boolean isIndexible(BaseRecord rec) {
		if(!model.equals(rec.getSchema()) && !rec.getSchema().equals(ModelNames.MODEL_INDEX_STORE)){
			logger.error("Record " + rec.getSchema() + " cannot be indexed by the " + model + " indexer");
			return false;
		}
		...
		if(!hasIdentity) {
			logger.error("Record " + rec.getSchema() + " does not define an identity field");
	...
	public IndexEntry addIndexEntry(BaseRecord rec) throws IndexException {
		...
		synchronized(indices) {
			...
			if(eidx != null) {
				...
				throw new IndexException(String.format(IndexException.INDEX_EXISTS, rec.getSchema()));
			}
