...
select distinct EI.model, P.id as actorId, P.name as actorName, R2.id as effectiveRoleId, R2.name as effectiveRoleName, R2.type as effectiveRoleType, R.id as baseRoleId, R.name as baseRoleName, R.type as baseRoleType, PR.id as permissionId, PR.name as permissionName, ER.id as groupId, ER.name as groupName, GO.model as objectSchema, GO.id as objectId, GO.name as objectName from effective${name}ActorRoles EI
