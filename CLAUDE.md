...
The SQLite/vector memory system (`.claude/memory/`, the `memory-db` MCP server, and the
SessionStart / UserPromptSubmit / Stop memory hooks) is **disabled as of 2026-08-31**. Do not
run `mem.ps1`, `vec.ps1`, or `export.ps1`, and do not attempt to read or write `memory.db`.

- The knowledge that was in the store is archived, read-only, at **`.claude/memory-archive.md`**
  (77 records). Consult it like any other reference doc; never write to it.
- Durable project guidance belongs in `CLAUDE.md`, `.claude/rules/*.md`, and `src/aiDocs/`.
- To re-enable: restore `.mcp.json` from `.mcp.json.disabled` and `.claude/settings.json` from
  `.claude/settings.json.bak-memory`.
